...
function drawLeaderboard() {
  ...
  for (var i = 0; i < leaders.length; i++) {
    text(
      leaders[i].score + " - " + leaders[i].name,
      GAME_WIDTH - 60,
      70 + i * 22 + 40
    );
  }
...
function bounce() {
  ...
  var offset =
    environment.scrollPosition + thePlayer.xpos + CHARACTER_WIDTH / 2;
  for (var i = 0; i < environment.trampolines.length; i++) {
    var diff = environment.trampolines[i] - offset;
    if (Math.abs(diff) < CHARACTER_WIDTH) {
      // less forgiving because of improved targeting
      return;
    }
  }
...
function increaseDifficulty() {
  if (environment.trampolinesPlaced % 5 == 0) {
    ...
    spawnQuote();
    if (environment.trampolinesPlaced % 10 == 0) spawnBalloon();
    if (environment.trampolinesPlaced % 15 == 0) spawnRainbow();
    if (environment.trampolinesPlaced % 20 == 0) spawnStrobe();
    if (environment.trampolinesPlaced % 25 == 0) spawnFounder();
  }
}
  ...
  switch (gameState) {
    ...
    case 1:
      ...
      var screenHit =
        thePlayer.xpos +
        CHARACTER_WIDTH / 2 +
        JUMP_DISTANCE * (1 - ((thePlayer.jumpTheta / Math.PI) % 1));
      noStroke();
      fill(color(255, 0, 0));
      rect(
        screenHit - TRAMPOLINE_WIDTH / 2,
        GAME_HEIGHT - 3,
        TRAMPOLINE_WIDTH,
        4
      );
