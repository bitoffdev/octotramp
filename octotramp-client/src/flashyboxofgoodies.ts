...
export function spawnQuote() {
  ...
  var quote =
    INSPIRATIONAL_STRINGS[int(Math.random() * INSPIRATIONAL_STRINGS.length)];

  activeQuote.text = quote;

  //determine starting location by randomizing a spot along the valid edges
  var start = int(
    Math.random() * (STRINGS_ABOVE * 2 + GAME_WIDTH - STRINGS_WITHIN_EDGE)
  );
  ...
  //coming from top
    ...
    if (topPos < STRINGS_WITHIN_EDGE) activeQuote.x = topPos;
    else activeQuote.x = topPos + STRINGS_WITHIN_EDGE;
  ...
  for (var i = 0; activeQuotes && i < activeQuotes.length; i++) {
    ...
    switch (current.direction) {
      ...
      case "left":
        ...
        break;
      case "down":
        if (current.x < GAME_WIDTH) textAlign(LEFT);
    ...
    if (
      current.x < 0 ||
      current.x > GAME_WIDTH ||
      current.y < 0 ||
      current.y > GAME_HEIGHT
    ) {
      activeQuotes.pop();
      i--;
    }
...
export function continueBalloon(thePlayer: Player, balloonImage: Image) {
  ...
  balloonX += 5;
  var balloonXPos =
    (sin((3 * frameCount) / thePlayer.jumpDuration) + 1) * 100 + balloonX;
  var balloonYPos =
    ((cos((3 * frameCount) / thePlayer.jumpDuration) + 1) * GAME_HEIGHT) / 2 -
    balloonY;

  image(balloonImage, balloonXPos, balloonYPos, 100, 100);

  if (balloonX > GAME_WIDTH) balloonActive = false;
}
...
export function continueRainbow(rainbowImage: Image) {
  ...
  if (
    rainbowXs[rainbowXs.length - 1] + RAINBOW_WIDTH > GAME_HEIGHT &&
  ...
  if (rainbowXs[rainbowXs.length - 1] > GAME_HEIGHT) {
    ...
    while (rainbowXs.length > 0) rainbowXs.pop();
  }
...
export function strobe() {
  ...
  if (frameCount - strobeStart > STROBE_FRAMES) strobeActive = false;
}
...
export function continueFounder(githubFounderImage: Image) {
  ...
  if (founderX > GAME_WIDTH) founderActive = false;
  pop();
}
